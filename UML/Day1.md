...
```plantuml
@startuml
title Hamlet Decision Engine

start

:Input Moral Dilemma;

:Activate Ghost Signal Receiver;
if (Ghost demands revenge?) then (Yes)
  :Increase Pressure;
else (No)
  :Maintain Internal Conflict;
endif

:Enter Doubt Generator;
:Amplify Uncertainty;

:Pass through Conscience Filter;
if (Is action morally justified?) then (Yes)
  :Consider Action;
else (No)
  :Return to Overthinking Loop;
endif

:Overthinking Loop;
while (Still uncertain?) is (Yes)
  :Analyze Outcomes;
  :Re-evaluate Ethics;
  :Increase Anxiety;
endwhile (No)

if (Decision reached?) then (Yes)
  :Execute Action (Delayed);
  :Trigger Chaos Module;
else (No)
  :No Action Taken;
endif
...
<img width="393" height="1104" alt="image" src="https://github.com/user-attachments/assets/30ac617d-fb4f-4694-82da-014cb6ae10d4" />
